...
const DashboardGerente = () => {
  ...
  const name       = fcaStorage.get('name') || 'Gerente';
  ...
  useEffect(() => {
    if (!fcaStorage.get('token') || role !== 'gerente') navigate('/login/GH');
  }, [navigate, role]);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setSubs(sub.direct);
      setWinData(win);
      setAvailable((dash.visible_users || []).filter((u) => u.role === 'coordenacao' && !u.manager_id));
    } catch (err) { toast.error(err.message); }
  }, []);

  useEffect(() => { loadData(); }, [loadData]);

  const totalSupervisores = subordinates.reduce((acc, c) => acc + (c.subordinates?.length || 0), 0);
  const filteredHierarchy = subordinates.filter((coordenador) => hierMatchNode(coordenador, hierSearch));
          ...
          <ContentArea>
            ...
            {tab === 'Minha Equipe' && (
              <>
                ...
                <MetricsRow>
                  ...
                  <Metric>
                    <div className="label">Supervisores (diretos)</div>
                    <div className="value">{totalSupervisores}</div>
                  </Metric>
                  <Metric>
                ...
                <Card>
                  <CardLabel>Coordenadores vinculados a você</CardLabel>
                  <TblWrap style={{ marginTop: '0.8rem' }}>
                    <Tbl>
                      <thead>
                        <tr>
                          <th>Nome</th><th>Matrícula</th><th>Regional</th>
                          <th>Empresa</th><th>Supervisores</th>
                        </tr>
                      </thead>
                      <tbody>
                        {subordinates.map((u) => (
                          <tr key={u.id}>
                            ...
                            <td>
                              <span style={{ background: 'rgba(47,122,63,0.14)', color: '#1a5028', borderRadius: '999px', padding: '2px 10px', fontSize: '0.76rem', fontWeight: 700 }}>
                                {u.subordinates?.length || 0} supervisor{(u.subordinates?.length || 0) !== 1 ? 'es' : ''}
                              </span>
                            </td>
                          </tr>
                        ))}
                        {subordinates.length === 0 && (
                          <tr>
                            <td colSpan={5}>
                              <Empty><div className="icon">👥</div>Nenhum coordenador vinculado ainda.</Empty>
                            </td>
                          </tr>
                        )}
                      </tbody>
            ...
            {tab === 'Hierarquia' && (
              <>
                ...
                <MetricsRow>
                  <Metric>
                    <div className="label">Coordenadores vinculados</div>
                    <div className="value">{subordinates.length}</div>
                  </Metric>
                  <Metric>
                    <div className="label">Supervisores na hierarquia</div>
                    <div className="value">{totalSupervisores}</div>
                  </Metric>
                </MetricsRow>
                ...
                <Card>
                  <CardRow>
                    <CardLabel>Coordenadores, supervisores e técnicos vinculados</CardLabel>
                  ...
                  {filteredHierarchy.length === 0 && (
                    <Empty style={{ marginTop: '1.4rem' }}>
                      ...
                      {subordinates.length > 0
                        ...
                        : 'Nenhum coordenador vinculado a você ainda.'}
                    </Empty>
                  )}

                  <div style={{ marginTop: '1.2rem', display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                    {filteredHierarchy.map((coordenador) => (
                      <HierNode key={coordenador.id} node={coordenador} search={hierSearch} />
                    ))}
                  </div>
                </Card>
              </>
            ...
            {tab === 'Vincular' && (
              <>
                <PageTitle>Vincular <em>Coordenadores</em></PageTitle>
                          ...
                          <tr>
                            <td colSpan={7}>
                              <Empty>
                                <div className="icon">👥</div>
                                {available.length > 0
                                  ? 'Nenhum coordenador encontrado para esta regional / busca.'
                                  : 'Nenhum coordenador disponível para vínculo no momento.'}
                              </Empty>
                            </td>
                          </tr>
                        )}
